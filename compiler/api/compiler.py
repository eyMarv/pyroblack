...
def get_return_type_hint(qualtype: str) -> str:
    ...
    if qualtype.startswith("Vector"):
        ...
        return f'"List[raw.base.{f"{ns}.{name}".strip(".")}]"'
    ns, name = qualtype.split(".") if "." in qualtype else ("", qualtype)
    return f'"raw.base.{f"{ns}.{name}".strip(".")}"'
...
def start(format: bool = False) -> None:
    ...
    for line in schema:
        ...
        if combinator_match:
            ...
            name = camel(name)
            qualname = f"{namespace}.{name}".lstrip(".")

            typespace, type = qualtype.split(".") if "." in qualtype else ("", qualtype)
            type = camel(type)
            qualtype = f"{typespace}.{type}".lstrip(".")
    ...
    for c in combinators:
        ...
        arguments = (", *, " if c.args else "") + (
            ", ".join(
                [f"{i[0]}: {get_type_hint(i[1])}" for i in sorted_args],
            )
            if sorted_args
            else ""
        )
