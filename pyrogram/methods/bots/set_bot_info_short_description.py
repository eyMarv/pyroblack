...
class SetBotInfoShortDescription:
    async def set_bot_info_short_description(
        ...
        The short description is shown on the bot's profile page and sent together with the bot link.

        .. note::

            If the current account is an User, can be called only if the ``for_my_bot`` has
            ``can_be_edited`` property set to True.
        ...
        Parameters:
            ...
            for_my_bot (``int`` | ``str``, *optional*):
                Unique identifier (int) or username (str) of the bot for which the short description
                should be changed instead of the current bot.
