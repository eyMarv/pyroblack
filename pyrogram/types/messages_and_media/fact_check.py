...
class FactCheck(Object):
    ...
    Parameters:
        need_check (``bool``, *optional*):
            If set, the country/text fields will not be set, and the fact check must be fetched manually by the client
            using bundled messages.getFactCheck requests when the message scrolls into view.
    ...
    def _parse(
        ...
        text, entities = (
            utils.parse_text_with_entities(
                client, getattr(fact_check, "text", None), users
            )
        ).values()
