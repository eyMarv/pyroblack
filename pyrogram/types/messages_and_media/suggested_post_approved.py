...
class SuggestedPostApproved(Object):
    ...
    @staticmethod
    async def _parse(client, message) -> "SuggestedPostApproved":
        action = message.action
        if getattr(action, "rejected", None) or getattr(action, "balance_too_low", None):
            return None
        ...
        return SuggestedPostApproved(
            ...
            send_date=utils.timestamp_to_datetime(getattr(action, "schedule_date", None)),
        )
