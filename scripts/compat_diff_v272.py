...
class CallableInfo:
    ...
    params: List[str]
    return_type: Optional[str]  # annotation as unparsed source, or None
    is_async: bool
...
def read_workspace(path: str, cwd: Path) -> Optional[str]:
    p = cwd / path
    if not p.is_file():
        return None
...
def extract_callables(src: str, file: str) -> List[CallableInfo]:
    ...
    try:
        ...
        raise SyntaxError(f"{file}: {e}") from e
...
def normalize_return(rt: Optional[str]) -> Optional[str]:
    if rt is None:
        return None
    s = rt.strip()
...
def compare(
    ...
    if skip_raw:
        old_files = [f for f in old_files if "/raw/" not in f and not f.startswith("pyrogram/raw/")]

    new_files_set = {
        str(p.relative_to(cwd)).replace("\\", "/")
        for p in (cwd / root).rglob("*.py")
    } if (cwd / root).exists() else set()
    if skip_raw:
        new_files_set = {
            f for f in new_files_set if "/raw/" not in f and not f.startswith("pyrogram/raw/")
        }
    ...
    if not include_private:
        all_new = [
            c
            for c in all_new
            if not c.name.startswith("_") or c.name in ("__init__", "__call__", "__aenter__", "__aexit__")
        ]
...
def print_report(report: Report, verbose: bool) -> None:
    ...
    if report.moved_callables and verbose:
        w("-" * 72 + "\n")
        w(f"MOVED CALLABLES ({len(report.moved_callables)}) — still present elsewhere\n")
        w("-" * 72 + "\n")
